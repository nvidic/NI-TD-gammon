...
class Model(object):
    def __init__(self, sess, model_path, summary_path, checkpoint_path, restore=False):
        #sta su ove putanje?
        #saving and restoring variables mozda
        self.model_path = model_path
        self.summary_path = summary_path
        #Variables are saved in binary files that, roughly, contain a map from variable names to tensor values. - checkpoint file
        self.checkpoint_path = checkpoint_path

        # setup our session
        self.sess = sess
        #sta bese znaci trainable=False? osim sto se ova varijabla ne dodaje u graf
        ...
        #ne znam sta je ova promenljiva ?
        global_step_op = self.global_step.assign_add(1)

        # get gradients of output V wrt trainable variables (weights and biases)
        #wrt - with regard to; tf.gradients(ys, xs, ...) - Constructs symbolic partial derivatives of sum of ys w.r.t. x in xs
        ...
        with tf.variable_scope('apply_gradients'):
            for grad, var in zip(grads, tvars):
                with tf.variable_scope('trace'):
                    ...
                    trace_op = trace.assign((lamda * trace) + grad) #assign(ref, value, ...): Update 'ref' by assigning 'value' to it.
                ...
                grad_apply = var.assign_add(grad_trace) #assign_add(ref, value, ...): Update 'ref' by adding 'value' to it.
                apply_gradients.append(grad_apply)

        # as part of training we want to update our step and other monitoring variables
        #control_dependencies(control_inputs) :
        #                         control_inputs: A list of Operation or Tensor objects which must be executed or computed before running
        #                                         the operations defined in the context. Can also be None to clear the control dependencies.
        with tf.control_dependencies([
            ...
            #tf.group(*inputs, **kwargs): Create an op that groups multiple operations. When this op finishes, all ops in input have finished.
            # This op has no output. Returns: operation that executes all its inputs
